...
public class EmpData {
	...
	public int[][] parseGeogDataFile(String fileLocation){
        ...
        public void parsePLS(String fileLocation){
            File file=new File(fileLocation+"PLSmeans.csv");
		...
		while((line=reader.readLine())!=null){	
                    String[] s=line.split(cvsSplitBy);
                    if (s.length==1) {
			buf[c]=Double.parseDouble(s[0]);
                        c++;
                    }
		}
                means=new double[c];
                System.arraycopy(buf, 0, means, 0, c);
		...
		reader.close();			
            ...
            file=new File(fileLocation+"PLSsds.csv");
		...
		while((line=reader.readLine())!=null){	
			...
			buf[c]=Double.parseDouble(s[0]);
                ...
                sds=new double[c];
                System.arraycopy(buf, 0, sds, 0, c);
		...
		reader.close();			
            ...
            file=new File(fileLocation+"PLSloadingweights.csv");
		...
		while((line=reader.readLine())!=null){	
                    String[] s=line.split(cvsSplitBy);
                    buf[c]=new double[s.length];
                    for (int i=0; i<s.length; i++){
                        buf[c][i]=Double.parseDouble(s[i]);
                    }
                    c++;
		}
                loadings=new double[c][];
                for (int i=0; i<c; i++){
                    loadings[i]=new double[buf[i].length];
                    System.arraycopy(buf[i], 0, loadings[i], 0, loadings[i].length);
                }
	...
	public String[][] parseLabelFile(String fileLocation){
				...
				last=s[0];
                        ...
                        */
                        ...
                        for (int i=0; i<locs.size(); i++){
                            String[] s1=locs.get(i);
                            boolean found=false;
                            for (int j=0; j<i; j++){
                                String[] s2=locs.get(j);
                                if (s1[0].equals(s2[0])){
                                    found=true;
                                    j=i;
                                }
		...
		*/
            ...
            System.out.println("NumInds "+out.length+" "+count);
		return out;
	}
